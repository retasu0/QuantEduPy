...
リポジトリのさらなる充実に向けて、以下の5つの柱に基づく拡充案が検討されています。
詳細は [docs/FUTURE_CONTENT_PROPOSALS.md](FUTURE_CONTENT_PROPOSALS.md) を参照してください。

1. **シミュレーションモデルの高度化**: ジャンプ拡散モデル、ヘストンモデル、マートン・モデルの導入。
2. **歴史的イベントに基づくケーススタディ**: 過去の金融危機（リーマンショック等）の再現とファットテールの理解。
3. **インタラクティブな学習ツールの導入**: Streamlitデモアプリや用語集の整備。
4. **演習とゲーミフィケーション**: ポートフォリオ構築チャレンジや自動採点クイズの追加。
5. **AIチューター機能の拡張**: 対話型解説やコード生成補助の強化。
